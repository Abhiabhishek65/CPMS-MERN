...
async function downloadNoticesReport(req, res) {
  const notices = await Notice.find({ createdByTpo: req.user.id }).sort({ createdAt: -1 }).lean();
  const headers = ["Title","Preview","Attachment","CreatedAt","UpdatedAt"];
  ...
  res.setHeader("Content-Disposition", 'attachment; filename="notices_summary.csv"');
