...
async function downloadApplicantsReport(req, res) {
  ...
  const headers = [
    "JobTitle","Company","StudentName","StudentEmail","RollNo","Department","Year","Phone","ApprovedForPlacement","ApplicationStatus","AppliedAt"
  ];
  ...
  const rows = apps.map(a => ([
    ...
    a.student?.name,
    a.student?.email,
    excelText(a.student?.rollNo),
    a.student?.department,
    a.student?.year,
    excelText(a.student?.phone),
    a.student?.isApproved ? "YES" : "NO",
    a.status,
  ...
  res.setHeader("Content-Disposition", `attachment; filename="${safeTitle}_applicants.csv"`);
