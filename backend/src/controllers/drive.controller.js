...
async function registerForDrive(req, res) {
  ...
  try {
    const reg = await DriveRegistration.create({ drive: driveId, student: req.user.id });
...
async function updateAttendance(req, res) {
  ...
  const allowed = ["REGISTERED", "PRESENT", "ABSENT"];
...
async function downloadDriveReport(req, res) {
  ...
  const headers = ["DriveTitle","Company","DriveDateTime","StudentName","Email","RollNo","Dept","Year","Approved","AttendanceStatus","RegisteredAt"];
  const rows = regs.map(x => ([
    x.drive?.title, x.drive?.company, x.drive?.dateTime ? new Date(x.drive.dateTime).toISOString() : "",
    x.student?.name, x.student?.email, x.student?.rollNo, x.student?.department, x.student?.year,
    x.student?.isApproved ? "YES" : "NO", x.status, x.createdAt ? new Date(x.createdAt).toISOString() : ""
  ]));
...
module.exports = {
  ...
  driveRegistrations, updateAttendance, downloadDriveReport, downloadDrivesSummaryReport
};
