...
export default function StudentProfile() {
  ...
  const [form, setForm] = useState({
    rollNo: user?.rollNo || "", department: user?.department || "", year: user?.year || "", placementYear: user?.placementYear || "",
  ...
  const save = async (e) => {
    ...
    try {
      const payload = {
        rollNo: form.rollNo, department: form.department, year: form.year, placementYear: form.placementYear, phone: form.phone,
  ...
  return (
    <div className="container">
      <div className="card" style={{ maxWidth: 760, margin: "0 auto" }}>
        <div className="row" style={{ justifyContent:"space-between" }}><h2>My Profile</h2><Link className="btn secondary" to="/student/dashboard">Back</Link></div>
        ...
        <form className="grid" onSubmit={save} style={{ marginTop: 10 }}>
          <div className="grid grid-2">
            ...
            <div><label>Phone</label><input className="input" value={form.phone} onChange={e=>onChange("phone", e.target.value)} /></div>
          </div>
          ...
          <div><label>Placement Year</label><select className="input" value={form.placementYear} onChange={e=>onChange("placementYear", e.target.value)}><option value="">Select Placement Year</option>{placementYears.map(year => <option key={year} value={year}>{year}</option>)}</select></div>
