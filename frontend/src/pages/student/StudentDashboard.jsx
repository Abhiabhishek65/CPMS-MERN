...
function QuickLink({ title, subtitle, to, showDot }) {
  return (
    <Link to={to} className="card quick-link-card" style={{ boxShadow: 'none', border: '1px solid #eee', display: 'block' }}>
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    api.get("/api/applications/my").then(r => setApps(r.data)).catch(e => setErr(e?.response?.data?.message || "Failed to load applications"));
  }, []);
    ...
    <div className="container">
      <div className="card">
        <div className="row" style={{ justifyContent: "space-between" }}>
          ...
          <div className="row"><Link className="btn" to="/jobs">Browse Jobs</Link><Link className="btn secondary" to="/student/profile">Profile</Link><Link className="btn secondary" to="/student/interviews"><span className="nav-with-dot">Interviews{updates.interviews ? <span className="notif-dot" aria-hidden="true" /> : null}</span></Link></div>
        </div>
        ...
        <div className="kpi" style={{ marginTop: 12 }}>
          ...
          {user?.approvalNote ? <div style={{ marginTop: 8 }}><small className="muted">Note: {user.approvalNote}</small></div> : null}
        </div>

        <div className="grid grid-2" style={{ marginTop: 14 }}>
          <QuickLink title="Jobs" subtitle="New jobs indicator available" to="/jobs" showDot={updates.jobs} />
        ...
        <table className="table">
          <thead><tr><th>Job</th><th>Company</th><th>Status</th><th>Note</th></tr></thead>
          <tbody>
            {apps.map(a => <tr key={a._id}><td><Link to={`/jobs/${a.job?._id}`}>{a.job?.title}</Link></td><td>{a.job?.company}</td><td><span className="badge">{a.status}</span></td><td>{a.note || "-"}</td></tr>)}
            {apps.length === 0 ? <tr><td colSpan="4"><small className="muted">No applications yet.</small></td></tr> : null}
          </tbody>
        </table>
      </div>
    </div>
