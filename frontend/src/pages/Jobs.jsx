          ...
          <tbody>
            {jobs.map(j => (
              <tr key={j._id}>
                ...
                <td>{j.salary || "-"}</td>
                <td><div className="row"><Link className="btn secondary" to={`/jobs/${j._id}`}>View</Link>{role === "tpo" ? <Link className="btn secondary" to={`/tpo/jobs/${j._id}/edit`}>Edit</Link> : null}{role === "tpo" ? <button className="btn secondary" onClick={() => deleteJob(j._id)}>Delete</button> : null}</div></td>
              </tr>
            ))}
