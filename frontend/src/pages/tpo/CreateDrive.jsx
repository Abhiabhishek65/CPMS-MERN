    ...
    <div className="container">
      <div className="card" style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <form className="grid" onSubmit={submit} style={{ marginTop: 10 }}>
          ...
          <div><label>Eligible Courses</label><select className="input" multiple value={form.eligibleDepartments} onChange={onCourseChange} style={{ minHeight: 180 }}>{COURSE_OPTIONS.map(course => <option key={course} value={course}>{course}</option>)}</select><small className="muted">Ctrl/Command dabake ek se jyada course select kar sakte ho.</small></div>
          ...
          <small className="muted">CGPA based students ke liye CGPA compare hoga, aur percentage based students ke liye percentage compare hoga.</small>
